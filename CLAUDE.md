...
- **中英双语 · 每模式严格单语言(默认英文)**：`I18N={en,zh}` 词表 + `applyLang(l)` 统一刷新所有 UI(标题/note/pills/强度/昼夜/实况按钮+状态/复位提示/`document.title`/`<html lang>`)。
  动态文本也走词表：现象标题 `updateHead()`、实况状态 `liveStatText()`(由 `lastLive` 重渲染，切语言不重新请求)、WMO 现在天气名(未命中走 `codePrefix`)。
  ⚠️ **铁律：英文模式零中文、中文模式零拉丁词**。所以：pill 只显示当前语言主标(`.en` 已 `display:none`，不再叠另一语言)；
  切换器 `#langtog` 标签也随语言(`langLabels`：英模式 `EN/ZH`、中模式 `英/中`)；`zh` 词表里不留品牌词(去掉了 Cesium/OSM/Open-Meteo/LIVE/ON/console)。
  新增 UI 文案务必同时加到 `I18N.en` 和 `I18N.zh`，且 **en 值不含 CJK、zh 值不含拉丁词**(数字/%/°/· 除外；Cesium 自带的 credits 水印是第三方必留归属，非应用文案)。
- **UI 无障碍/响应式约定(/audit 修过，别回退)**：现象 pill 是 `<button>`(非 div)，选中态用 `syncPills()` 同步 `.on`+`aria-pressed`(在 `buildSystem` 里调)；
  langtog/dn/fronttog 切换也带 `aria-pressed`；`:focus-visible` 全局焦点环；`#liveStat` 是 `aria-live`；slider 有 `aria-labelledby`。
  CSS 用 token(`--r-lg/md/sm`、`--surface-active`、`--accent-live`、`--ease`)；`@media(max-width:680px)` 让 dock `flex-wrap` 不溢出、`@media(pointer:coarse)` 加大触摸目标、`@media(prefers-reduced-motion)` + `recenter()` 减弱动效。**新控件要带 button 语义 + aria + token，别用纯 div + 魔法数。**
